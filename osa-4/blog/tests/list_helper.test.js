...
describe('total likes', () => {
  ...
  test('when list has many blogs sums the likes of all', () => {
...
describe('favorite blog', () => {
  test('seek favorite from many blogs', () => {
    const result = listHelper.favoriteBlog(manyBlogs)
    expect(result).toEqual(manyBlogs[2])
  })
})

describe('most blogs by writer', () => {
  test('check which writer has most blogs', () => {
    const result = listHelper.mostBlogs(manyBlogs)
    expect(result).toEqual({ author: 'Robert C. Martin', blogs: 3 })
  })
})

describe('most likes for one writer', () => {
  test('check which writer has most likes overall', () => {
